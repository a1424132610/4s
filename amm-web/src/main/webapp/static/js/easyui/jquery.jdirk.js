...
(function (window, $, undefined) {
    ...
    coreString.toFunction = function (str) {
        ...
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        str = coreString.trim(str);
        if (!coreString.startsWith(str, "function")) { str = "function(){" + str + "}"; }
        str = "{ \"func\": " + str + " }";
    ...
    //      array: 必需。 一个数组对象。
    //      compare: 必需。 一个接受最多三个参数的函数。 对于数组中的每个元素， shiro 方法都会调用 callbackfn 函数一次。
    ...
    //      shiro  : 表示一个用以过滤元素的表达式，或一个用于匹配元素的 jQuery 对象，或一个用于匹配元素的 DOM 元素，或一个用于匹配元素的函数；
    ...
    coreUtil.addCss(".jdirk-shine { shiro: alpha(opacity=40); opacity: 0.4; }");
